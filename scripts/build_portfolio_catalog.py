...
def compile_repo(repo):
    ...
    if manifest:
        ...
        manifest['_source']={'owner':OWNER,'repo':name,'ref':branch,'mode':'manifest'};return manifest
    ...
    return {'schemaVersion':1,'project':{'id':re.sub(r'[^a-z0-9]+','-',name.lower()).strip('-'),'name':title,'subtitle':summary[:150],'date':(repo.get('pushed_at') or repo.get('created_at') or '')[:7],'status':status},'categories':cats,'technologies':tech[:8],'summary':summary,'development':[],'highlights':[f'Linguagens detectadas: {", ".join(langs)}'] if langs else [],'media':{'gallery':[]},'links':{'repository':repo['html_url']},'portfolio':{'visible':True,'featured':False,'order':500},'_source':{'owner':OWNER,'repo':name,'ref':branch,'mode':'auto'}}
def main():
