const PortfolioProjects = (() => {
    ...
    function rawBase(source) { return `https://raw.githubusercontent.com/${source.owner}/${source.repo}/${source.ref}/`; }
    function resolveAsset(source, path) { if (!path) return ''; if (/^https?:\/\//i.test(path)) return path; return rawBase(source) + path.replace(/^\//, ''); }
    function projectHref(project) { return `projeto.html?id=${encodeURIComponent(project.project.id)}`; }
    function validateProject(data) { return Boolean(data && data.schemaVersion === 1 && data.project?.id && data.project?.name && Array.isArray(data.categories) && Array.isArray(data.technologies) && data.portfolio); }

    async function loadConfig() { const response = await fetch(configUrl, { cache: 'no-store' }); if (!response.ok) throw new Error('Configuração do catálogo indisponível'); return response.json(); }
    ...
    function createCard(data) {
        ...
        const card = document.createElement('a'); card.className = `project manifest-project ${categories.join(' ')}`; card.href = projectHref(data); card.dataset.label = `${categories.join(' + ').toUpperCase()} // ${source.mode === 'manifest' ? 'MANIFEST' : 'AUTO'}`; card.dataset.projectId = data.project.id;
    ...
    async function renderProjectGrid() {
        ...
        try { const projects=await loadCatalog(); const visible=projects.filter(p=>p.portfolio?.visible!==false).sort((a,b)=>(a.portfolio?.order??999)-(b.portfolio?.order??999)); grid.innerHTML=''; visible.forEach(p=>grid.appendChild(createCard(p))); if(typeof applyFilters==='function')applyFilters(); if(typeof setupRevealAnimations==='function')setupRevealAnimations(); }
        catch(error){ console.error(error); grid.innerHTML=`<div class="project-load-error">CATALOG ERROR // ${escapeHtml(error.message)}</div>`; }
    }

    async function renderProjectDetail() {
        const host=document.getElementById('manifest-project-detail'); if(!host)return; const id=new URLSearchParams(window.location.search).get('id');
        if(!id){host.innerHTML='<div class="project-load-error">PROJECT DATA ERROR // projeto não informado.</div>';return;}
        host.innerHTML='<div class="project-loading">LCARS // carregando registro do projeto…</div>';
        try { const projects=await loadCatalog(); const data=projects.find(p=>p.project.id===id); if(!data)throw new Error('Projeto não encontrado no catálogo compilado'); document.title=`${data.project.name} — Jorge Bandeo`; host.innerHTML=''; host.appendChild(createDetail(data)); }
        catch(error){host.innerHTML=`<div class="project-load-error">PROJECT DATA ERROR // ${escapeHtml(error.message)}</div>`;}
    }
    ...
    function createDetail(data) {
        ...
        if(data.caseStudy&&window.ProjectCaseStudy){const wrap=document.createElement('div');wrap.innerHTML=window.ProjectCaseStudy.render(data);article.appendChild(wrap.firstElementChild);}
    ...
    function addLink(host,href,label){if(!href)return;const link=document.createElement('a');link.href=href;link.target='_blank';link.rel='noopener noreferrer';link.textContent=label;host.appendChild(link);}
    function createMediaItem(item,source,projectName){const figure=document.createElement('figure');figure.className='manifest-media-card';const placeholder=document.createElement('div');placeholder.className='media-placeholder';placeholder.textContent=item.type==='video'?'VIDEO SLOT // aguardando upload':'IMAGE SLOT // aguardando upload';figure.appendChild(placeholder);if(item.src&&source.owner&&source.repo&&source.ref){let media;if(item.type==='video'){media=document.createElement('video');media.controls=true;media.preload='metadata';media.src=resolveAsset(source,item.src);}else{media=document.createElement('img');media.loading='lazy';media.src=resolveAsset(source,item.src);media.alt=item.caption||projectName||'Imagem do projeto';}media.addEventListener('error',()=>media.remove());figure.appendChild(media);}if(item.caption){const caption=document.createElement('figcaption');caption.textContent=item.caption;figure.appendChild(caption);}return figure;}
    function escapeHtml(value=''){return String(value).replace(/[&<>'"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char]));}
    document.addEventListener('DOMContentLoaded',()=>{renderProjectGrid();renderProjectDetail();}); return{loadCatalog,resolveAsset};
})();
